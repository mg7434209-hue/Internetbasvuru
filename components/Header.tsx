...
export default function Header() {
  ...
  return (
    <>
      {/* Dealer Strip — telefon kaldırıldı, ortalı, kurumsal görünüm */}
      ...
      {/* Main Header — telefon kaldırıldı, logo büyütüldü, sade ve etkili */}
      <header className="bg-white/85 backdrop-blur-md px-[5%] py-4 flex justify-between items-center sticky top-0 z-[1000] border-b border-black/5">
        {/* Logo: büyük TT logosu + ince ayrıcı + site adı */}
        <Link
          href="/"
        ...
        >
          ...
          <span className="hidden md:inline-flex flex-col leading-tight border-l border-ink-100 pl-3">
            <span className="text-[10px] font-bold text-ink-500 tracking-[0.12em] uppercase">
              Yetkili Bayi
            </span>
            <span className="text-sm font-bold text-ink-900 mt-0.5">
              internetbaşvuru
            </span>
          </span>
        </Link>

        {/* Right: Nav + CTA (telefon kaldırıldı) */}
        <div className="flex items-center gap-4 md:gap-6">
          ...
          <button
            ...
            className="bg-ink-900 hover:bg-brand-500 text-white px-5 py-2.5 rounded-full font-bold text-sm transition-all hover:-translate-y-0.5"
          >
            Hemen Başvur
          </button>
        </div>
